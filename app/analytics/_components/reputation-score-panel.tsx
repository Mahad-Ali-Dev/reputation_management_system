...
export function ReputationScorePanel({
  ...
  const band =
    score >= 80 ? { label: "Excellent", color: "var(--ok)" } :
    score >= 60 ? { label: "Good", color: "var(--pri)" } :
    score >= 40 ? { label: "Fair", color: "var(--warn)" } :
    { label: "Needs work", color: "var(--bad)" };

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div className="ds-card">
        <div className="ds-card__body" style={{ display: "flex", gap: 24, alignItems: "center", flexWrap: "wrap" }}>
          <ScoreRing value={score} color={band.color} />
          <div style={{ minWidth: 220, flex: 1 }}>
            <div style={{ fontSize: 13, color: "var(--rl-muted)" }}>Reputation score</div>
            <div style={{ fontSize: 22, fontWeight: 700, color: band.color, marginTop: 2 }}>{band.label}</div>
            <p style={{ fontSize: 13, color: "var(--rl-muted)", marginTop: 8, marginBottom: 0, maxWidth: 460, lineHeight: 1.5 }}>
              ...
              unlock the final {factors.filter((f) => !f.available && (f.key === "citation_consistency" || f.key === "local_pack")).reduce((s, f) => s + f.weight, 0)} points.
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, marginBottom: 4 }}>
        <span style={{ color: locked ? "var(--rl-muted-2)" : "var(--ink)", fontWeight: 500, display: "inline-flex", alignItems: "center", gap: 6 }}>
          {factor.label}
          {locked && (
            <span style={{ display: "inline-flex", alignItems: "center", gap: 3, color: "var(--rl-muted-2)", fontSize: 11 }}>
      ...
      <div style={{ height: 8, borderRadius: 4, background: "var(--surface-3)", overflow: "hidden" }}>
        <div
          style={{
            ...
            background: locked ? "var(--rl-muted-3)" : pct >= 70 ? "var(--ok)" : pct >= 40 ? "var(--warn)" : "var(--bad)",
          }}
        />
      </div>
    </div>
