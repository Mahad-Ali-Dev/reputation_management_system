    ...
    <details className="ds-card">
      ...
      >
        {snapshot.execSummary && (
          <p style={{ fontSize: 13.5, color: "var(--ink)", margin: 0, lineHeight: 1.5 }}>
            {snapshot.execSummary}
          </p>
        )}
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap", fontSize: 13 }}>
          <Stat
            label="Local pack"
            value={snapshot.localPackPosition != null ? `#${snapshot.localPackPosition}` : "—"}
          />
          <Stat
            label="Website sessions"
            value={
              snapshot.websiteSessions != null ? snapshot.websiteSessions.toLocaleString() : "—"
            }
          />
        </div>
        {factors.length > 0 && (
