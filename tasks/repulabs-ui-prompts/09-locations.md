...
## SCREEN: Locations (multi-location management)
For chains/franchises managing many storefronts. Two views.

**A. Locations list** — header with "Add location" CTA and a search/filter bar.
Cards or table per location: name, address, status chip (Active/Pending/Needs
attention), avg rating + StarRating, total reviews, response rate, connected-source
icons. A small summary strip up top: total locations, group avg rating, lowest-
performing location callout. Row click → location detail.

**B. Location detail / settings** — tabbed:
  • Overview — that location's mini-dashboard (KPIs, recent reviews, trend).
  • Profile — name, address, phone, hours, category, map placeholder.
  • Connected sources — per-platform connect status for this location.
  • Team — staff assigned to this location.
  • Campaign defaults — default review-request template for this location.

Include a header with location switcher, breadcrumbs back to list, loading
skeletons, an empty state (no locations yet → add first location), and an
add/edit-location modal or drawer with validation. Responsive throughout.

## OUTPUT
1) Short rationale. 2) Complete self-contained React+TS+Tailwind Locations list +
detail (tabbed) + add/edit drawer. 3) States & assumptions note.
